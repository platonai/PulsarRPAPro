...
You can use Exotic to generate all the scraping rules automatically and then use the web console to manage the crawler in detail:

. STEP1: Extract almost every field in any webpage automatically using our advanced AI
. STEP2: Check the automatically generated SQLs, test and modify it to match the most concerned needs if necessary
. STEP3: Create a crawl rule in the web console to run the SQLs continuously
...
[source,bash]
----
java -jar exotic-standalone*.jar harvest https://shopee.sg/Computers-Peripherals-cat.11013247 -diagnose
----
...
== Scrape pages use generated SQL
The harvest command extracts fields automatically using unsupervised machine learning, and also generates the best css selectors for all possible fields and the SQL. We can execute the SQL query using the sql command.
[source,bash,sql]
----
...
from load_out_pages('https://shopee.sg/(Local-Stock)-(GEBIZ-ACRA-REG)-PLA-3D-Printer-Filament-Standard-Colours-Series-1.75mm-1kg-i.182524985.8326053759?sp_atk=3afa9679-22cb-4c30-a1db-9d271e15b7a2&xptdk=3afa9679-22cb-4c30-a1db-9d271e15b7a2', 'div.page-product');
"
----

== Run the standalone server and open web console
Finally, we have the perfect scrape rules to match our most complex requirement, we can use the web console to manage the crawler in detail and run the crawlers continuously.
