...
. STEP2: check the automatically generated scraping SQLs, test and modify it to match the further needs if necessary
. STEP3: create crawl rules in the web console to run the scraping SQLs continuously
...
The harvest command extracts fields automatically using unsupervised machine learning, and also generates the best css selectors for all possible fields and the scraping SQLs. We can execute the SQL query using the `sql` command.
[source,bash,sql]
----
...
Finally, we have the perfect scraping rules to match our most complex requirement, we can use the web console to manage the crawlers and run them on demand.
