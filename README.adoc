...
*#Never write another web scraper. Exotic generates all the extract rules automatically and manages the crawlers in detail:#*

. STEP1: automatically extract every field in webpages using advanced AI and generate extract SQLs
. STEP2: test and improve the SQLs to match business requirements if necessary
...
Finally, we have the perfect extract rules to match our most complex requirements, we can use the web console to manage the crawlers and run them on demand.
