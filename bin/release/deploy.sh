...
#scp -r "$APP_HOME"/exotic-standalone/target/exotic-standalone-"$VERSION".jar "master:$REMOTE_BASE_DIR"

echo "List directory before creating symbolic link:"
ssh master ls -l $REMOTE_BASE_DIR
ssh master unlink $REMOTE_BASE_DIR/exotic-standalone.jar
ssh master ln -s $REMOTE_BASE_DIR/exotic-standalone-"$VERSION".jar $REMOTE_BASE_DIR/exotic-standalone.jar
